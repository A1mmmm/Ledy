...
if (bookingForm) {
    bookingForm.addEventListener('submit', e => {
        ...
        [['entry.1205287458', name], ['entry.1211161903', phone], ['entry.638195446', service]]
...
if(prevBtn) prevBtn.addEventListener('click', () => { if(currentImageIndex>0) showImage(currentImageIndex-1); });
if(nextBtn) nextBtn.addEventListener('click', () => { if(currentImageIndex<images.length-1) showImage(currentImageIndex+1); });

serviceCards.forEach(card => {
    card.addEventListener('click', () => {
        const title = card.querySelector('h3')?.innerText || '';
        const desc = card.querySelector('p')?.innerText || '';
        const serviceImages = ["img/ЗАГЛУШКА.png", "img/ЗАГЛУШКА.png"];

        document.getElementById('serviceTitle').innerText = title;
        document.getElementById('serviceDescription').innerText = desc;

        images.forEach((img, i) => img.src = serviceImages[i] || serviceImages[0]);
